...
window.onkeydown = function (e) {
   ...
   else if (key === 83) { // 's' for slow motion
      if (snailBait.slowMotionEnabled === undefined) {
         snailBait.slowMotionEnabled = false;
      }

      if (!snailBait.slowMotionEnabled) {
         snailBait.timeSystem.setTransducer(function (elapsedTime) {
            return elapsedTime * 0.25;
         });
         snailBait.slowMotionEnabled = true;
      } else {
         snailBait.timeSystem.setTransducer(function (elapsedTime) {
            return elapsedTime;
         });
